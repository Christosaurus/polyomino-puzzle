...
const LINES: Line[] = [
  ...
  { who: "mira", text: "Ich hab die Splitter im Boden nachgezählt. Werden wieder weniger — und das nicht durch uns.", ok: (c) => c.region === 0 && c.panes >= 4 },
  { who: "mira", text: "Ein Fenster nach dem anderen. Genau so hat's der Meister immer gesagt.", ok: (c) => c.region === 0 },
  // ── Mira, Werkstatt ────────────────────────────────────────────────────
  { who: "mira", text: "In der Werkstatt riecht's noch nach heißem Blei. Als wär er nur kurz raus.", ok: (c) => c.region === 1 && !c.sawTwist },
  { who: "mira", text: "Diese sauberen Schnitte. Kein Sturm macht das. Das macht eine Hand.", ok: (c) => c.region === 1 && !c.sawTwist },
  { who: "mira", text: "Ich weiß jetzt, wer die Gestalt ist. Ich will's nur nicht glauben.", ok: (c) => c.region >= 1 && c.sawTwist },
  ...
  { who: "anselm", text: "Jedes Fenster, das du schließt, ist eins weniger für mich. Das ist … in Ordnung.", ok: (c) => c.sawTwist },
  ...
  { who: "anselm", text: "Nur noch das letzte Fenster. Ich halte den Rahmen, wenn's soweit ist.", ok: (c) => c.sawTwist && c.panes >= 40 },
  { who: "mira", text: "Das Tal ist hell. Wir haben's tatsächlich gemacht.", ok: (c) => c.finished },
];
